...
def make_classifier_tagger(weights):
    ...
    def classify(words, all_tags):
        """This nested function should return a list of tags,
        computed using a classifier with the weights passed as arguments to make_classifier_tagger
        and using basefeatures for each token (just the token and the offset)
        ...
        for i in words:
            pos,_ = classifier_base.predict({i:1},weights,all_tags)
            pos_tags.append(pos)
